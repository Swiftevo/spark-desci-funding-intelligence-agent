...
function Invoke-FetchWebResource {
  param([string]$Url)
  ...
  if ($isGitHub) {
    ...
    try {
      ...
      $githubPattern = 'github\.com/[a-zA-Z0-9_.-]+/[a-zA-Z0-9_.-]+'
...
function Invoke-SearchAcademicContext {
  ...
  if ($useOpenAlexFallback) {
    ...
    $openAlexQuery = [System.Net.WebUtility]::UrlEncode($topic)
    $openAlexSelectFields = "id,title,display_name,publication_year,authorships,primary_location,doi,open_access,cited_by_count,topics"
    $openAlexUri = "https://api.openalex.org/works?search=$openAlexQuery&per_page=5&select=$openAlexSelectFields"
    if ($env:OPENALEX_API_KEY) {
      $encodedOpenAlexApiKey = [Uri]::EscapeDataString($env:OPENALEX_API_KEY)
      $openAlexUri += "&api_key=$encodedOpenAlexApiKey"
    }
...
$tools = @(
  ...
  [ordered]@{
    ...
    function = [ordered]@{
      ...
      description = "Fetch and analyze external web resources like GitHub repositories or project websites. Returns repository stats (stars, forks, activity), license info, and key findings for verifying project claims."
      parameters = [ordered]@{
        type = "object"
        properties = [ordered]@{
          url = [ordered]@{
            type = "string"
            description = "URL to fetch, e.g. 'https://github.com/user/repo' or project website"
          }
        }
        required = @("url")
      }
...
- Use fetch_web_resource to check GitHub repositories for: stars, forks, recent activity, license, and actual code presence.
- Use fetch_web_resource to check project websites for: accessibility, team info, and demo availability.
- If a project claims to have a GitHub repo or website, verify it actually exists and is active.
